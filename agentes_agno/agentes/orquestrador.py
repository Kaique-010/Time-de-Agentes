...
agente_orquestrador = Agent(
    ...
    instructions=[
        ...
        "Normalização de serializers é obrigatória: não expor campos legados na API. Ex.: titu_empr -> empresa, titu_fili -> filial, titu_clie -> cliente, titu_titu -> titulo, titu_venc -> vencimento, titu_valo -> valor, titu_noss_nume -> nosso_numero, titu_linh_digi -> linha_digitavel, titu_url_bole -> url_boleto",
        "Em serializers: usar aliases amigáveis com source='campo_legado', read_only/write_only quando necessário e validar consistência entre campos relacionados",
        ...
        "Em services: ser proativo com regras úteis ao domínio (saneamento de filtros, paginação, ordenação permitida, tratamento de nulos, métricas de atraso, transições seguras de status e transações atômicas em operações críticas)",
